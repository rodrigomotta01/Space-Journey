...
class Player(pygame.sprite.Sprite):
    def __init__(self, groups):
        ...
        self.rect = self.image.get_frect(center = (W_WIDTH / 2, W_HEIGHT / 2))
...
def display_score():
    ...
    time_score = (pygame.time.get_ticks() - start_time) // 100
    score_display = time_score + score
    text_surf = font.render(("score " + str(score_display)), True, '#ffffff')
...
#region Imports
#sprite import
meteor_surf = pygame.image.load(join("images", "meteor.png")).convert_alpha() #join to work both in Windows and Linux
    ...
    if game_started == 1:
        ...
        for event in pygame.event.get():
            ...
            if event.type == meteor_event:
                x, y = random.randint (100, W_WIDTH-100), random.randint (-200,-150)
